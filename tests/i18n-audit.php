...
$plugin_path     = $repository_root . '/indexlane-redirect-internal-link-auditor.php';
$include_paths   = glob( $repository_root . '/includes/*.php' );
...
if ( false === $plugin_source || false === $script || false === $include_paths || empty( $include_paths ) ) {
...
$php_sources = array( $plugin_path => $plugin_source );
foreach ( $include_paths as $include_path ) {
	$include_source = file_get_contents( $include_path );
	if ( false === $include_source ) {
		fwrite( STDERR, 'Could not read production include: ' . basename( $include_path ) . "\n" );
		exit( 1 );
	}
	$php_sources[ $include_path ] = $include_source;
}
...
foreach ( $php_sources as $source_path => $source ) {
	...
	for ( $index = 0, $token_count = count( $tokens ); $index < $token_count; $index++ ) {
		...
		foreach ( $requirements['literal_arguments'] as $argument_index ) {
			if ( ! isset( $arguments[ $argument_index ] ) || null === indexlane_i18n_literal_value( $arguments[ $argument_index ] ) ) {
				$errors[] = sprintf( '%s: %s() line %d must use a literal string for argument %d.', basename( $source_path ), $function_name, $token[2], $argument_index + 1 );
			}
		}
		...
		if ( $domain !== $call_domain ) {
			$errors[] = sprintf( '%s: %s() line %d must use the literal text domain %s.', basename( $source_path ), $function_name, $token[2], $domain );
		}
		...
		if ( null !== $primary_literal && preg_match( '/(?<!%)%(?:[0-9]+\$)?[-+0-9.]*[bcdeEfFgGosuxX]/', $primary_literal ) ) {
			if ( null === $last_translator_comment || $token[2] - $last_comment_end_line > 2 ) {
				$errors[] = sprintf( '%s: %s() line %d has placeholders without an adjacent translators comment.', basename( $source_path ), $function_name, $token[2] );
			}
		}
...
if ( false === strpos( implode( "\n", $php_sources ), 'placeholder="<?php esc_attr_e(' ) ) {
